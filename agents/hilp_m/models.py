"""
Models for HILP agent. Follows the implementation from:
https://github.com/seohongpark/HILP
"""
import torch
import math
from typing import Dict, Tuple
from agents.base import MLPEncoder, AbstractMLP
...
class AbstractPhi(AbstractMLP):
    ...
    def forward(self, goal: torch.Tensor, **kwargs) -> torch.Tensor:
        """
        ...
        z = self.trunk(goal)  # pylint: disable=E1102
...
class LaplacianFeatures(torch.nn.Module):
    ...
    def forward(self, goal: torch.Tensor, **kwargs) -> torch.Tensor:
        ...
        Args:
            goal: tensor of shape [batch_dim, observation_dim]
        Returns:
            phi: embedded feature tensor of shape [batch_dim, z_dimension]
        """

        return self.phi.forward(goal=goal)
    ...
    def get_loss(
        ...
        z = self.phi.forward(goal=observations)
        next_z = self.phi.forward(goal=next_observations)
...
class HILPFeatures(torch.nn.Module):
    """Feature embedding learned with HILP loss."""
    ...
    def __init__(
        ...
        self.phi1 = AbstractPhi(*phi_config)
        self.phi2 = AbstractPhi(*phi_config)
        self.target_phi1 = AbstractPhi(*phi_config)
        self.target_phi2 = AbstractPhi(*phi_config)
    ...
    def get_loss(
        ...
        Args:
            observations: tensor of shape [batch_dim, observation_dim]
            next_observations: tensor of shape [batch_dim, observation_dim]
            future_observations: tensor of shape [batch_dim, observation_dim]
        Returns:
            loss: loss for HILP update
            metrics: dictionary of metrics for logging
        """

        goals = future_observations
        rewards = (torch.linalg.norm(observations - goals, dim=-1) < 1e-6).float()
        masks = 1.0 - rewards
        rewards = rewards - 1.0

        next_v1, next_v2 = self._value(next_observations, goals, is_target=True)
        next_v = torch.minimum(next_v1, next_v2)
        q = rewards + self.discount * masks * next_v

        v1_t, v2_t = self._value(observations, goals, is_target=True)
        v_t = (v1_t + v2_t) / 2
        adv = q - v_t

        q1 = rewards + self.discount * masks * next_v1
        q2 = rewards + self.discount * masks * next_v2
        v1, v2 = self._value(observations, goals, is_target=False)
        v = (v1 + v2) / 2
        ...
        with torch.no_grad():
            phi1 = self.phi1.forward(observations)
    ...
    def _value(
        self, observations: torch.Tensor, goals: torch.Tensor, is_target: bool = False
    ):
        """
        ...
        Args:
            observations: tensor of shape [batch_dim, observation_dim]
            goals: tensor of shape [batch_dim, observation_dim]
        ...
        phi1_s = phi1.forward(observations)
        phi1_g = phi1.forward(goals)

        phi2_s = phi2.forward(observations)
        phi2_g = phi2.forward(goals)
