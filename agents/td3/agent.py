        ...
        if self.discrete_actions:
            ...
            if sample:
                action = (
                    ...
                    if np.randoom.rand() < self.exploration_epsilon
                    else action
                )
    ...
    def _update_critic(
        ...
        with torch.no_grad():
            if self.discrete_actions:
                ...
                target_Q1, target_Q2 = self.critic_target(next_observations)
                next_Q = torch.min(target_Q1, target_Q2)
                # target Q is greedy action
                target_Q = next_Q.max(1)[0].unsqueeze(1)
        ...
        if self.discrete_actions:
            current_Q1, current_Q2 = self.critic(observations)
