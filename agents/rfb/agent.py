...
"""Module defining the contextual Forward-Backward Agent."""
...
from agents.rfb.models import RecurrentForwardBackwardRepresentation
...
class RFB(AbstractAgent):
    """
    Recurrent forward-backward agent
    """
    ...
    def __init__(
        ...
        if not discrete_actions:
            if recurrent_F:
                ...
                elif memory_type == "fart":
                    self.actor = FARTActor(
                        observation_length=observation_length,
                        action_length=action_length,
                        z_length=z_dimension,
                        preprocessed_dimension=preprocessed_dimension,
        ...
        assert self._history_length > 0, "RFB requires non-zero history length."
    ...
    def _actor_inference(
        ...
        if self.memory_type in ["fart", "transformer"]:
