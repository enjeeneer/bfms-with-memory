...
def pull_model_from_wandb(
    ...
    if algorithm in ("vcfb", "mcfb", "vcalfb", "mcalfb"):
        ...
        from agents.cql.agent import CQL  # pylint: disable=import-outside-toplevel
        ...
        handshake_agent = CQL(
            ...
            cql_target_penalty=config["cql_target_penalty"],
        )

    elif algorithm in ("sf-hilp", "sf-hilp-probe"):

        from agents.sf.agent import SF  # pylint: disable=import-outside-toplevel
        ...
        if "point_mass_maze" in config["domain_name"]:
            config["discount"] = 0.99
            config["z_dimension"] = 100
        ...
        handshake_agent = SF(
            ...
            name=config["name"],
            z_mix_ratio=config["z_mix_ratio"],
            q_loss=config["domain_name"] in ("quadruped", "jaco"),
        ...
        handshake_agent.successor_net.load_state_dict(
            trained_agent.successor_net.state_dict()
        )
        handshake_agent.feature_net.load_state_dict(
            trained_agent.feature_net.state_dict()
        )
        handshake_agent.actor.load_state_dict(trained_agent.actor.state_dict())

    elif algorithm == "td3":

        from agents.td3.agent import TD3  # pylint: disable=import-outside-toplevel
...
def get_popgym_env(domain: str):
    """
    Gets popgym environment from domain name.
    Args:
        domain: popgym domain name
    Returns:
        Env
    """

    if domain == "StatelessCartPoleHard":
        env = popgym.envs.position_only_cartpole.PositionOnlyCartPoleHard()
        return DiscreteAction(env)

    elif domain == "NoisyStatelessCartPoleHard":
        env = popgym.envs.noisy_position_only_cartpole.NoisyPositionOnlyCartPoleHard()
        return DiscreteAction(env)

    elif domain == "StatelessPendulumHard":
        env = popgym.envs.position_only_pendulum.PositionOnlyPendulumHard()
        return env

    elif domain == "NoisyStatelessPendulumHard":
        env = popgym.envs.noisy_position_only_pendulum.NoisyPositionOnlyPendulumHard()
        return env

    elif domain == "RepeatPreviousHard":
        env = popgym.envs.repeat_previous.RepeatPreviousHard()
        return DiscreteAction(env)

    elif domain == "RepeatPreviousEasy":
        env = popgym.envs.repeat_previous.RepeatPreviousEasy()
        return DiscreteAction(env)
    elif domain == "RepeatPreviousMedium":
        env = popgym.envs.repeat_previous.RepeatPreviousMedium()
        return DiscreteAction(env)

    else:
        raise ValueError(f"Popgym env: {domain} not currently supported.")
